...
function PokemonReader.decryptSubstructure(address, key, index)
    ...
    for i = 0, 5 do  -- 12 bytes = 6 16-bit values
        ...
        -- XOR with key
        local decryptedValue = bit.bxor(encrypted, key)
        decrypted[i * 2 + 1] = bit.band(decryptedValue, 0xFF)
        decrypted[i * 2 + 2] = bit.rshift(decryptedValue, 8)
    end
...
function PokemonReader.readPokemon(address, isInPC)
    ...
    local key = bit.bxor(pokemon.personality, pokemon.otId)
    ...
    pokemon.parsedIVs = {
        hp = bit.band(pokemon.ivs, 0x1F),
        attack = bit.band(bit.rshift(pokemon.ivs, 5), 0x1F),
        defense = bit.band(bit.rshift(pokemon.ivs, 10), 0x1F),
        speed = bit.band(bit.rshift(pokemon.ivs, 15), 0x1F),
        spAttack = bit.band(bit.rshift(pokemon.ivs, 20), 0x1F),
        spDefense = bit.band(bit.rshift(pokemon.ivs, 25), 0x1F),
        isEgg = bit.band(bit.rshift(pokemon.ivs, 30), 0x1) == 1,
        isNicknamed = bit.band(bit.rshift(pokemon.ivs, 31), 0x1) == 1
    }
    
    -- Parse origins info
    pokemon.metLevel = bit.band(pokemon.origins, 0x7F)
    pokemon.gameOfOrigin = bit.band(bit.rshift(pokemon.origins, 7), 0xF)
    pokemon.ball = bit.band(bit.rshift(pokemon.origins, 11), 0xF)
    pokemon.otGender = bit.band(bit.rshift(pokemon.origins, 15), 0x1)
    ...
    if romData then
        if bit.band(pokemon.personality, 1) == 0 then
...
function PokemonReader.parseStatus(status)
    ...
    -- Sleep counter (0-7)
    local sleepCounter = bit.band(status, PokemonReader.statusFlags.SLEEP)
    if sleepCounter > 0 then
        conditions.sleep = sleepCounter
    end
    
    -- Other conditions
    if bit.band(status, PokemonReader.statusFlags.POISON) > 0 then
        conditions.poison = true
    end
    if bit.band(status, PokemonReader.statusFlags.BURN) > 0 then
        conditions.burn = true
    end
    if bit.band(status, PokemonReader.statusFlags.FREEZE) > 0 then
        conditions.freeze = true
    end
    if bit.band(status, PokemonReader.statusFlags.PARALYSIS) > 0 then
        conditions.paralysis = true
    end
    if bit.band(status, PokemonReader.statusFlags.BAD_POISON) > 0 then
