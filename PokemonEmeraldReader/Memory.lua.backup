...
function Memory.getDomain(addr)
    ...
    -- Use bit shift operator if available, fallback to bit.rshift
    local domain
    if _VERSION >= "Lua 5.3" then
        domain = addr >> 24
    else
        domain = rshift(addr, 24)
    end
    
    return Memory.domains[domain]
end

-- Convert GBA address to domain-specific offset
function Memory.getOffset(addr)
    local domain = Memory.getDomain(addr)
    
    if domain == "ROM" then
        -- ROM domain expects offset from 0, not full GBA address
        return addr - 0x08000000
    elseif domain == "EWRAM" then
    ...
    elseif domain == "OAM" then
        return addr - 0x07000000
    else
        -- For other domains or System Bus, use full address
        return addr
    end
end
...
function Memory.read_u8(addr)
    ...
    -- Get domain-specific offset
    local offset = Memory.getOffset(addr)
    
    -- Check if we need System Bus for EWRAM beyond 32KB
    if Memory.requiresSystemBus(addr) then
        domain = "System Bus"
        offset = addr  -- System Bus uses full address
        Memory.stats.systemBusFallbacks = Memory.stats.systemBusFallbacks + 1
    end
    
    -- Try primary domain
    local success, value = pcall(memory.read_u8, offset, domain)
...
function Memory.read_u16_le(addr)
    ...
    local success, value = pcall(memory.read_u16_le, offset, domain)
    if success and value then 
        return value 
    end
...
function Memory.read_u32_le(addr)
    ...
    local success, value = pcall(memory.read_u32_le, offset, domain)
    if success and value then 
        return value 
    end
